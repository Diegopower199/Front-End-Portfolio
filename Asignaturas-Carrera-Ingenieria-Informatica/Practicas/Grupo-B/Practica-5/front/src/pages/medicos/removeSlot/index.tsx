/*
  Explicacion de uso renderizado en el cliente: El renderizado es en cliente porque el usuario debe introducir valores en los input para eliminar un Slot, 
  por lo que el usuario esta interactuando con la pagina web, ademas despues de que lo elimine dando click al boton de eliminar, 
  se muestra abajo los datos que se han eliminado en el momento y ahí nos estamos creando nuevos elementos HTML.
  Tambien si no se se ha recargado la pagina tambien se muestra los anteriores.
  Ademas la pagina es dinamica ya que creamos nuevos elementos al eliminar usuarios para mostrar que datos hemos hemos eliminado y se ha realizado correctamente
*/
...
const RemoveSlot = () => {
  ...
  const [informacionRemove, setInformacionRemove] = useState<
    { year: number; month: number; day: number; hour: number }[]
  >([]);

  const [mutateRemoveCorrect, setmutateRemoveCorrect] =
  ...
  return (
    <>
      ...
      {mutateRemoveCorrect === true && (
        <>
          <h4>Informacion eliminada</h4>

          {informacionRemove.map((info, index) => {
            return (
              <>
                <DivElementosSlot>
